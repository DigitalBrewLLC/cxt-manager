...
exports.autoHealCommand = void 0;
const commander_1 = require("commander");
const chalk_1 = __importDefault(require("chalk"));
const core_1 = require("@cxtmanager/core");
exports.autoHealCommand = new commander_1.Command('auto-heal')
    ...
    try {
        ...
        if (ifNeeded) {
            ...
            if (!hasAutoFixableIssues) {
                ...
                if (!silent) {
                    console.log(chalk_1.default.green('✅ No fixes needed - all context files are aligned!'));
                }
                return;
            }
        }
        if (!silent) {
            console.log(chalk_1.default.blue(`🔧 ${dryRun ? 'Previewing' : 'Applying'} auto-heal fixes...`));
            console.log('');
        }
        ...
        if (!silent) {
            console.log(chalk_1.default.bold(`${dryRun ? '📋 Would make these changes:' : '✅ Applied fixes:'}`));
            ...
            console.log('');
            if (dryRun) {
                console.log(chalk_1.default.blue('💡 Run without --dry-run to apply these fixes'));
            }
            else {
                console.log(chalk_1.default.green('🎉 Auto-heal completed successfully!'));
                console.log(chalk_1.default.gray('📝 Changes have been committed to Git'));
                console.log('');
                console.log(chalk_1.default.blue('💡 Run "cit validate" to verify all issues are resolved'));
            }
        }
        else if (fixes.length > 0 && !dryRun) {
            // Silent mode but fixes were applied - show minimal output
            console.log(chalk_1.default.green(`✅ Auto-healed ${fixes.length} issue(s)`));
        }
    }
    catch (error) {
        ...
        if (error.message.includes('Not a Git repository')) {
            ...
            console.error(chalk_1.default.red('❌ Auto-heal failed:'), error.message);
        }
